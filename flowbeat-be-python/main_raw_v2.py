...
def adjust_all_records(records, latest_device_record, anchor_dt=None):
    """
    Koreksi SEMUA rekor berdasarkan selisih waktu antara rekor terbaru
    di perangkat dengan waktu saat ini.
    """
    if anchor_dt is None:
        anchor_dt = datetime.now()
    
    # Hitung selisih waktu antara rekor terbaru perangkat dan waktu sekarang
    time_delta = anchor_dt - latest_device_record["datetime"]

    # Koreksi semua rekor
    for user_records in records:
        for record in user_records:
            record["datetime"] = record["datetime"] + time_delta
        ...
        try:
            ...
            if data.pairing:
                ...
                return {
                    ...
                    "mac_address": selected_device.address,
                    "device_name": selected_device.name,
                    "records": [r for u in records for r in u]  # flatten list-of-list ke list of dicts
                }
        finally:
